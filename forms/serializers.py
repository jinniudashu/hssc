from rest_framework import serializers
from .models import *
class A6207Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6207
        fields = '__all__'
class T4501Serializer(serializers.ModelSerializer):
    class Meta:
        model = T4501
        fields = '__all__'
class A6206Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6206
        fields = '__all__'
class A3103Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3103
        fields = '__all__'
class A6215Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6215
        fields = '__all__'
class Z6201Serializer(serializers.ModelSerializer):
    class Meta:
        model = Z6201
        fields = '__all__'
class T3404Serializer(serializers.ModelSerializer):
    class Meta:
        model = T3404
        fields = '__all__'
class A3110Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3110
        fields = '__all__'
class A6216Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6216
        fields = '__all__'
class A3105Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3105
        fields = '__all__'
class A6217Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6217
        fields = '__all__'
class T3405Serializer(serializers.ModelSerializer):
    class Meta:
        model = T3405
        fields = '__all__'
class A6202Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6202
        fields = '__all__'
class A6201Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6201
        fields = '__all__'
class A6204Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6204
        fields = '__all__'
class T6301Serializer(serializers.ModelSerializer):
    class Meta:
        model = T6301
        fields = '__all__'
class Fu_wuSerializer(serializers.ModelSerializer):
    class Meta:
        model = Fu_wu
        fields = '__all__'
class Wu_liu_gong_ying_shang_ji_ben_xin_xi_biaoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Wu_liu_gong_ying_shang_ji_ben_xin_xi_biao
        fields = '__all__'
class Yao_pin_ji_ben_xin_xi_biaoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Yao_pin_ji_ben_xin_xi_biao
        fields = '__all__'
class Z6205Serializer(serializers.ModelSerializer):
    class Meta:
        model = Z6205
        fields = '__all__'
class A3109Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3109
        fields = '__all__'
class A3108Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3108
        fields = '__all__'
class T4502Serializer(serializers.ModelSerializer):
    class Meta:
        model = T4502
        fields = '__all__'
class Ji_gou_ji_ben_xin_xi_biaoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Ji_gou_ji_ben_xin_xi_biao
        fields = '__all__'
class Zhi_yuan_ji_ben_xin_xi_biaoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Zhi_yuan_ji_ben_xin_xi_biao
        fields = '__all__'
class She_bei_ji_ben_xin_xi_biaoSerializer(serializers.ModelSerializer):
    class Meta:
        model = She_bei_ji_ben_xin_xi_biao
        fields = '__all__'
class Yong_yao_diao_cha_biaoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Yong_yao_diao_cha_biao
        fields = '__all__'
class T3003Serializer(serializers.ModelSerializer):
    class Meta:
        model = T3003
        fields = '__all__'
class T3002Serializer(serializers.ModelSerializer):
    class Meta:
        model = T3002
        fields = '__all__'
class Fu_wu_fen_gong_ji_gou_ji_ben_xin_xi_biaoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Fu_wu_fen_gong_ji_gou_ji_ben_xin_xi_biao
        fields = '__all__'
class A6208Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6208
        fields = '__all__'
class Z6233Serializer(serializers.ModelSerializer):
    class Meta:
        model = Z6233
        fields = '__all__'
class A5001Serializer(serializers.ModelSerializer):
    class Meta:
        model = A5001
        fields = '__all__'
class T4504Serializer(serializers.ModelSerializer):
    class Meta:
        model = T4504
        fields = '__all__'
class Shu_ye_zhu_she_danSerializer(serializers.ModelSerializer):
    class Meta:
        model = Shu_ye_zhu_she_dan
        fields = '__all__'
class A6219Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6219
        fields = '__all__'
class A6203Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6203
        fields = '__all__'
class A3001Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3001
        fields = '__all__'
class A3101Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3101
        fields = '__all__'
class A6501Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6501
        fields = '__all__'
class T4505Serializer(serializers.ModelSerializer):
    class Meta:
        model = T4505
        fields = '__all__'
class A6211Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6211
        fields = '__all__'
class A3502Serializer(serializers.ModelSerializer):
    class Meta:
        model = A3502
        fields = '__all__'
class A6218Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6218
        fields = '__all__'
class A6205Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6205
        fields = '__all__'
class A6214Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6214
        fields = '__all__'
class Z6230Serializer(serializers.ModelSerializer):
    class Meta:
        model = Z6230
        fields = '__all__'
class A6220Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6220
        fields = '__all__'
class A6212Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6212
        fields = '__all__'
class T9001Serializer(serializers.ModelSerializer):
    class Meta:
        model = T9001
        fields = '__all__'
class A5002Serializer(serializers.ModelSerializer):
    class Meta:
        model = A5002
        fields = '__all__'
class Physical_examination_athletic_abilitySerializer(serializers.ModelSerializer):
    class Meta:
        model = Physical_examination_athletic_ability
        fields = '__all__'
class A6213Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6213
        fields = '__all__'
class Z6261Serializer(serializers.ModelSerializer):
    class Meta:
        model = Z6261
        fields = '__all__'
class A6210Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6210
        fields = '__all__'
class A6209Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6209
        fields = '__all__'
class A6502Serializer(serializers.ModelSerializer):
    class Meta:
        model = A6502
        fields = '__all__'
class Can_hou_2_xiao_shi_xue_tangSerializer(serializers.ModelSerializer):
    class Meta:
        model = Can_hou_2_xiao_shi_xue_tang
        fields = '__all__'
class Xue_ya_jian_ce_ping_guSerializer(serializers.ModelSerializer):
    class Meta:
        model = Xue_ya_jian_ce_ping_gu
        fields = '__all__'
